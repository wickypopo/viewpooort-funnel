...
function sanitizePayload(body) {
  return {
    ...
    consent:
      body.consent && typeof body.consent === "object" ? body.consent : null,
    tracking:
...
function createWeb3FormsPayload(lead) {
  return {
    access_key: process.env.VITE_FORM_ACCESS_KEY,
    ...
    problems:
      lead.problems.length > 0 ? lead.problems.join(", ") : "Nicht angegeben",
    ...
    message: [
      ...
      lead.problems.length > 0
        ? lead.problems.map((problem) => `- ${problem}`).join("\n")
        : "Nicht angegeben",
      "",
      "Consent:",
...
export default async function handler(request, response) {
  ...
  if (!process.env.VITE_FORM_ACCESS_KEY) {
  ...
  if (!web3FormsResponse.ok || result?.success === false) {
    return response.status(502).json({
      ...
      providerStatus: web3FormsResponse.status,
    });
  }
